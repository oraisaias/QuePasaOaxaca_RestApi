...
@Injectable()
export class CategoriaAuthGuard extends AuthGuard('jwt') {
  ...
  handleRequest(
    ...
    // Permitir acceso a usuarios de la app y administradores
    const allowedRoles = ['admin', 'app_user', 'app_user_logged'];
    if (!allowedRoles.includes(user.role)) {
      throw new UnauthorizedException({
        statusCode: 403,
        message: 'Acceso denegado',
        error: 'Forbidden',
        details: 'Este endpoint solo está disponible para usuarios autenticados',
      });
    }
